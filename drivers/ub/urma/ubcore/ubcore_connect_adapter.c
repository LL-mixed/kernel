...
static inline void ubcore_apply_tpid_ctx(struct ubcore_active_tp_cfg *cfg,
	const struct ubcore_tpid_ctx *ctx)
{
	cfg->tp_handle.value = ctx->tp_handle;
	cfg->peer_tp_handle.value = ctx->peer_tp_handle;
	cfg->tp_attr.tx_psn = ctx->tx_psn;
	cfg->tp_attr.rx_psn = ctx->rx_psn;
}
	...
	struct ubcore_tpid_key *key, struct ubcore_active_tp_cfg *cfg)
{
	...
	struct ubcore_tpid_ctx *ctx = NULL;
	struct ubcore_hash_table *ht;
	union ubcore_tp_handle old_tp_handle;
	uint32_t hash;
	...
	if (ctx && !ctx->is_init) {
		old_tp_handle = cfg->tp_handle;
		ubcore_apply_tpid_ctx(cfg, ctx);
		spin_unlock(&ht->lock);
		ubcore_log_info("Find tpid in target, hash: %u.\n", hash);
		(void)ubcore_deactive_tp(dev, old_tp_handle, NULL);
		kfree(add_ctx);
		return;
	}
	...
	struct ubcore_udata *udata)
{
	...
	/* Reuse */
	ubcore_apply_tpid_ctx(&active_tp_cfg, ctx);
	active_tp_cfg.tag = 0;
	...
	struct ubcore_tjetty *tjetty, struct ubcore_udata *udata)
{
	...
	if (ret == 0 && tjetty->cfg.tp_type == UBCORE_RTP)
		ubcore_fadd_init_tpid_ctx(dev, &key, &active_tp_cfg);
